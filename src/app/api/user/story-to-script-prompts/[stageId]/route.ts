...
import {
  STORY_TO_SCRIPT_EDITABLE_STAGE_MAP,
  type StoryToScriptEditableStageId,
} from '@/lib/novel-promotion/story-to-script-stage-prompts'

function parseStageId(value: string): StoryToScriptEditableStageId {
  const trimmed = value.trim() as StoryToScriptEditableStageId
  if (!STORY_TO_SCRIPT_EDITABLE_STAGE_MAP.has(trimmed)) {
...
export const GET = apiHandler(async (
  ...
  const stage = STORY_TO_SCRIPT_EDITABLE_STAGE_MAP.get(stageId)!
